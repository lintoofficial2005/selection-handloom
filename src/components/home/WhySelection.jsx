...
export const WhySelection = () => {
  ...
  return (
    <section className="py-12 sm:py-20 bg-[#F3EFE6]/60 border-t border-[#E6EDE6]">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
          {pillars.map((p, idx) => {
            ...
            return (
              <div
                key={p.title}
                className="p-5 sm:p-6 bg-white rounded-2xl border border-[#D0DDD1] shadow-xs hover:border-[#8DA98F] transition-all space-y-2 sm:space-y-3"
              >
                <div className="w-10 h-10 rounded-xl bg-[#E6EDE6] text-[#243528] flex items-center justify-center">
                  <Icon className="w-5 h-5" />
                </div>
                <h4 className="font-brand-title text-sm sm:text-base font-bold text-[#1E2A21]">
                  {p.title}
                </h4>
