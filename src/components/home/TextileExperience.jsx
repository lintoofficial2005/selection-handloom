...
export const TextileExperience = () => {
  ...
  return (
    <section className="py-24 bg-[#1E2A21] text-[#FAF7F2] relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center max-w-2xl mx-auto mb-16">
          ...
          <p className="text-xs sm:text-sm text-[#FAF7F2]/70 mt-3 max-w-lg mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-center">
          ...
          <div className="lg:col-span-7">
            <div className="relative rounded-2xl overflow-hidden shadow-2xl border border-[#3D503F] aspect-16/10 bg-[#29382C] group">
              <img
                key={current.name}
                src={current.image}
                alt={current.name}
              ...
              <div className="absolute bottom-6 left-6 right-6 p-4 rounded-xl bg-[#1E2A21]/90 backdrop-blur-md border border-[#8DA98F]/30 flex items-center justify-between">
                <div>
                  ...
                  <span className="font-brand-title text-base font-bold text-white">
                    {current.name}
                  </span>
                </div>
                <div className="flex items-center gap-3 text-xs text-[#FAF7F2]/80">
                  <span className="px-2.5 py-1 bg-[#29382C] rounded-lg border border-[#3D503F]">
                    {current.gsm}
                  </span>
                  <span className="px-2.5 py-1 bg-[#29382C] rounded-lg border border-[#3D503F]">
              ...
              {textures.map((tex, idx) => (
                ...
                >
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-xs uppercase tracking-widest text-[#AEC4B0] font-semibold">
                      0{idx + 1} • {tex.name}
                    </span>
                    {activeTexture === idx && (
                  ...
                  <h4 className="font-brand-title text-sm sm:text-base font-bold text-white">
                    {tex.title}
                  </h4>
                  {activeTexture === idx && (
